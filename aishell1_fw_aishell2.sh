...
checkpoint=/data/experiment/wenet/examples/aishell/s2/exp_gpu_A1_fw_A2/69.pt
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  for ((i = 0; i < $num_gpus; ++i)); do
  {
    ...
    python train.py --gpu $gpu_id \
      ...
      --train_data /data/wenet/examples/aishell/s1/utils/aishell1_whisperaishell2/data.list  \
      --cv_data /data/wenet/examples/aishell/s1/utils/dev/data.list  \
      ${checkpoint:+--checkpoint $checkpoint} \
      --model_dir exp_gpu_A1_fw_A2/ \
      --num_workers 1 \
      --cmvn /data/wenet/examples/aishell/s1/utils/aishell1_whisperaishell2/global_cmvn \
      --pin_memory
  } &
  done
  wait
